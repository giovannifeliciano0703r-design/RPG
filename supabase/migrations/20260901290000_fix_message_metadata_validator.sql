-- Keep this replacement as a distinct migration because the validator was
-- already deployed before the JSON array iterator ambiguity was discovered.
...
begin
  if jsonb_typeof(value) <> 'object'
    or value - array['senderName','senderAvatar','characterId','rollData','imageUrl'] <> '{}'::jsonb then return false; end if;
  ...
  if value ? 'imageUrl' and (jsonb_typeof(value -> 'imageUrl') <> 'string' or char_length(value ->> 'imageUrl') > 2000 or value ->> 'imageUrl' not like 'https://%') then return false; end if;
  if value ? 'rollData' then
    ...
    if jsonb_typeof(roll) <> 'object'
      ...
      or jsonb_typeof(roll -> 'total') <> 'number' then return false; end if;
